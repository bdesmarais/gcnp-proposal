...
We work through the description of our overall modeling framework using a graphical depiction of the most important aspects of the structure provided in Figure \ref{plates}. Let $K$ be the number of topics. Each topic is constituted by a multinomial distribution over words ($w$) with natural parameter $\beta$. The topic proportions ($\theta$) constituting the multinomial distribution over topics in a selected document are given by the mean transformation of the natural multinomial parameter ($\eta$). Inter-temporal and inter-domain dependence is constructed by drawing $\eta$ from a Gaussian vector autoregressiion model \cite{Banbura2010} (i.e., $\eta \sim \mathcal{N}\left(\alpha_t,\sigma^2I \right)$). Given $M$ domains, the mean natural parameter corresponding to topic $k$ in domain $m$ at time $t$ is $$ \alpha_{(t,m,k)} = \lambda_{(0,m,k)} + \lambda_{(1,1,k)}\alpha_{(t-1,1,k)}+ \lambda_{(2,m,k)}\alpha_{(t-1,2,k)}+\hdots+\lambda_{(M,m,k)}\alpha_{(t-1,M,k)},$$ where $ \lambda_{(0,m,k)}$ is an over-time intercept for the natural parameter corresponding to topic $k$ in domain $m$ and $\lambda_{(v,m,k)}$ is a real-valued parameter that gives the regression relationship between the natural parameter of topic $k$ in domain $v$ at time $t-1$ and the natural parameter of topic $k$ in domain $m$ at time $t$. This parameterization renders the natural parameter corresponding to a given topic (i.e., the prominence of a given topic) in a selected domain dependent upon the natural parameter of that same topic across all other domains in the immediate past. Through $\lambda$, this vector autoregressive model represents the dynamics of topic progression across domains.

use logistic-normal distributions in the construction of domain-specific topic distributions to model cross-domain relationships will constitute a highly flexible, modular and principled approach to tying the domains together and modeling the interdependencies among them. We will derive models with a common set of topics across domains, but varying attention to topics in each domain. The attention to each topic will be given by a logistic transformation of Gaussian-distributed attention parameters. For each topic $t$, there will be four domain-specific attentiveness parameters. The four Gaussian attention parameters will be modeled as a vector autoregression \cite{Banbura2010}. This will permit topics to be related across domains. Thus, the attention level to a topic in each domain will be related to the attention level to a topic in every other domain, up to a selected number of periods into the past. 
