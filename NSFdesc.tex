...
We integrate domain-specific models by connecting the attention to a topic across time and domains using a vector autoregression model ~\cite{Banbura2010}. Let
$K$ be the number of topics. Each topic $k$ is characterized by a
multinomial distribution over words with mean parameter vector
$\bm{\phi}_k$. The multinomial distribution over topics
...
\sum_{k'=1}^K \exp{(\eta_{k'|d})}$. We specify the model for $\bm{\eta}_d$ to capture inter-temporal and inter-domain dependence in topic attention as well as the effects of document-specific fixed (i.e., upstream \cite{Mimno2012}) metadata associated with document $d$, which we denote $\bm{x}_d$. We draw $\bm{\eta}_d$ from a Gaussian
vector autoregression model~\cite{Banbura2010}. That is,  given that document $d$ is associated with time $t$ and domain $m$, $\bm{\eta}_d
\sim \mathcal{N}\left(\bm{\alpha}_{t,m}+\bm{\gamma}_d, a^2 \mathbf{I} \right)$,
where $\bm{\alpha}_{t,m} \sim \mathcal{N}(\bm{\mu}_{t,m},\sigma^2
\mathbf{I})$, $\bm{\gamma}_{d} = f(\bm{x}_d)$, and $f(\cdot)$ is a function that conditions the topic distribution on $\bm{x}_d$ (e.g., Dirichlet multinomial regression \cite{Mimno2012}). In this section, we do not describe the specific structure of $f$ since it will be different for each domain. We do, however, give the complete specification of the model for $\bm{\mu}_{t,m}$. Given $M$ domains, $\mu_{k|t,m}$ is given by $$
...
\node (theta) at (1.25,.75) [circle, draw] {$\bm{\theta}_d$};
\node (z) at (0,-.45) [circle, draw] {$z_n$};
\node (y) at (1.25,-1.55) [circle, draw,fill=lightgray] {$\bm{y}_d$};
\node (x) at (0,.75) [circle, draw,fill=lightgray] {$\bm{x}_d$};
\node (gamma) at (1.25,-2.9) [circle, draw] {$\bm{\gamma_t}$};
\node (w) at (0,-1.55)  [circle, draw,fill=lightgray] {$w$};
\node(alpha) at (0,1.9) [circle, draw] {$\bm{\alpha}$};
\node (phi) at (-1.5-.2,-1.55) [circle, draw] {$\bm{\phi}$};
...
\draw[-latex] (z) -- (w);
\draw[-latex] (x) -- (theta);
%\draw[-latex] (x1.west) -- (theta1.east);
\draw[-latex] (z) -- (y);
%\draw[-latex] (z1.east) -- (tau1.west);
\draw[-latex] (gamma) -- (y);

%\draw (-.8,-3.35) rectangle (2,2.35);
\draw (-.7,-2.3) rectangle (1.85,1.4);
\draw (-.6,-2.25) rectangle (.625,.2);
...
\draw (-2.2-.2,-2.3) rectangle (-.8-.2,-1);
\node at (-1.1-.2,-2.1){$K$};

\end{tikzpicture}
\end{center}
\vspace{-.3cm}
\caption{Graphical representation of a dynamic multi-corpus model of text ($w$) and context ($y$ and $x$) over two time periods. Each topic's natural parameters ($\bm{\phi}$) evolve over time. The logistic normal determinants of topic proportions ($\alpha$) co-evolve across the $M$ domains.}
\vspace{-.15cm}
...
domain-specific model family will incorporate downstream metadata associated with each document. Unlike the fixed, upstream metadata ($\bm{x}_d$), on which the topic proportions are conditioned,
the upstream data, which we denote $\bm{y}_d$, are conditioned on topic attention and modeled according to an associated metadata model. To illustrate the difference between upstream and downstream models that incorporate metadata, we can contrast the author-recipient topic model \cite{McCallum2005} with the topic-partitioned latent space model from our pilot research; both of which constitute appropriate generative models for emails. In the author-recipient topic model, there is a different multinomial distribution over topics for each author-recipient pair, but there is no generative model for the authors or the recipients. Our topic-partitioned latent space model incorporates a topic-dependent generative social network model of author-recipient connections. Thus, the author-recipient topic model (i.e., the upstream model) predicts the topic of a message based on the author-recipient metadata, and the topic-partitioned latent space model (i.e., downstream) predicts author-recipient connections based on the content in a message. Each domain-specific model will incorporate some combination of upstream and downstream metadata, and we anticipate that important insights will be drawn from the comparison of upstream and downstream models for the same metadata. \vspace{.1cm}
...
the flow of topical foci throughout different domains in the
...
\item Draw topics $\bm{\phi}_t|\bm{\phi}_{t-1}\sim \mathcal{N}\left(\bm{\phi}_{t-1},\delta^2I \right)$
...
\item Draw $W \sim \text{Mult}(\pi(\bm{\phi}_{t,z}))$
\end{enumerate}
...
~\\
\noindent {\bf Modeling Contributions:} We will model citizen input by
...
the authors of outside input messages will send only a few
emails. To deal with author sparsity, it will be advantageous to aggregate over several
...
this clustering approach maps very well onto the concept of partisanship. This
...
input,  and (3) recipient-specific peculiarities of input content and (via the cross-time, cross-domain  vector autoregressive topic model) (4)
the over-time change in the content of input. A fascinating challenge in the development of this model class will be managing cluster dynamics. The problems of actor re-labeling, cluster creation and cluster elimination are open problems that must be confronted to use cluster models in a dynamic context \cite{Mucha2010}. 
...
~\\
\noindent {\bf Modeling contributions:} In this domain, we will make the
...
visualization and exploration.  This specification
will permit us to understand how the content and network context of
intra-governmental communications co-evolve. We will improve upon the model class developed in our pilot research in two ways. First, we will incorporate over-time (Markov) drift in the vertices' positions in the
latent space using the framework proposed by Sarkar and Moore for modeling latent space dynamics with dichotomous dynamic networks \cite{Sarkar2005}. This will enable us to forecast
evolution in communication network structure. Second, recent work has shown that inference via variational approximation drastically improves the scalability of the latent space model for social networks \cite{SalterTownshend2013}. We will develop and implement a version of variational approximation for use with our topic-partitioned latent space model of communication networks.
...
arise through the legislative bargaining process. This will
constitute an indispensable component of our input-response-feedback
modeling, but would also be applicable to countless
...
~\\
\noindent {\bf Modeling contributions:} We will augment the dynamic
topic model with a topic-specific network structure, whereby legislators are connected to positions on legislation (e.g., yea, nay, abstain). The network will,
